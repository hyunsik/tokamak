...
use common::codespan::{BytePos, Loc, FileLinesResult, FileName, MultiSpan, Span};
use emitter::{ColorConfig, Emitter, EmitterWriter};
...
impl CodeSuggestion {
  ...
  pub fn splice_lines(&self, cm: &CodeMapper) -> String {
    use common::codespan::{CharPos, Loc, Pos};
    ...
    let bounding_span = Span { lo: lo, hi: hi };

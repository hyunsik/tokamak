...
use common::plugin::{FuncRegistry, Plugin, PluginManager, TypeRegistry};
...
impl<'a> QueryRunner for LocalQueryRunner<'a> {
  ...
  fn execute(&self, session: &Session, plan: &Operator) -> Result<DataSet> {
    let logical_plan = try!(self.planner
                                .build(self.ty_registry(), self.fn_registry(), session, plan));
    let optimized = try!(self.optimizer.optimize(self.ty_registry(),
                                                 self.fn_registry(),
                                                 session,
                                                 &logical_plan));
    let exec_plan = try!(self.exec_planner
                             .build(self.ty_registry(), self.fn_registry(), session, &optimized));

...
impl<'a> ExecutionPlan<'a> {
  pub fn new(driver_factories: Vec<DriverFactory<'a>>) -> ExecutionPlan<'a> {
    ExecutionPlan { driver_factories: driver_factories }
  }
...
impl ExecutionPlanner {
  ...
  pub fn build(&self,
    ...
    debug_assert!(ctx.stack.len() == 1,
                  "Empty or remain one more factories in stack");

    let driver_factory = DriverFactory::new(true, true, ctx.stack.pop().unwrap());

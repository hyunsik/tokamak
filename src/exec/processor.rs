...
/// (input rows, input selected rows IDs, input selected count, output selected rows IDs)
...
impl<'a> MapCompiler<'a> {
  ...
  fn write_const_values(&'a self, bld: &'a Builder, func: &Function, exprs: &[&Expr]) {
    ...
    let const_exprs = izip!(0..exprs.len(), exprs).filter(|e| {
      match *e.1.kind() {
  ...
  fn write_nonconst_values(&'a self,
    ...
    let nonconst_exprs = izip!(0..exprs.len(), exprs).filter(|e| {
      match *e.1.kind() {
  ...
  fn create_fn_prototype(&self, bld: &Builder) -> Function {
    ...
    // Prototype: (&Page, &mut Page, SelectedList, usize) == (*Page, *Page, *const
    // usize, usize)
...
pub trait Processor
{
  fn process(&self, input: &Page, output: &mut Page) -> Void;
}
...
mod tests {
  ...
  pub fn codegen() {
    ...
    unsafe {
      for idx in 0..ROWBATCH_SIZE {
        ...
        assert_eq!((lhs_val + rhs_val) as i64,
                   c_api::read_i64_raw(out_page.chunk(2), idx));
      }
    }
  }
  ...
  pub fn tpch1() {
    ...
    let types = &[I64, // l_orderkey      bigint

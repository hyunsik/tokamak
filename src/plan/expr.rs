...
pub fn Cast(c: Expr, from_ty: &Ty, to_ty: &Ty) -> Expr {
  expr(BOOL,
       ExprKind::Cast(Box::new(c), from_ty.clone(), to_ty.clone()))
}
...
pub fn Plus(ret_type: &Ty, l: Expr, r: Expr) -> Expr {
  expr(ret_type,
       ExprKind::Arithm(ArithmOp::Plus, Box::new(l), Box::new(r)))
}

#[allow(non_snake_case)]
pub fn Sub(ret_type: &Ty, l: Expr, r: Expr) -> Expr {
  expr(ret_type,
       ExprKind::Arithm(ArithmOp::Sub, Box::new(l), Box::new(r)))
}

#[allow(non_snake_case)]
pub fn Mul(ret_type: &Ty, l: Expr, r: Expr) -> Expr {
  expr(ret_type,
       ExprKind::Arithm(ArithmOp::Mul, Box::new(l), Box::new(r)))
}

#[allow(non_snake_case)]
pub fn Div(ret_type: &Ty, l: Expr, r: Expr) -> Expr {
  expr(ret_type,
       ExprKind::Arithm(ArithmOp::Div, Box::new(l), Box::new(r)))
}

#[allow(non_snake_case)]
pub fn Rem(ret_type: &Ty, l: Expr, r: Expr) -> Expr {
  expr(ret_type,
       ExprKind::Arithm(ArithmOp::Rem, Box::new(l), Box::new(r)))
}
...
pub fn Switch(cases: Vec<Expr>, default: Expr) -> Expr {
  Expr(default.ty().clone(),
       ExprKind::Switch(vec::to_boxed(cases), Box::new(default)))
}

#[allow(non_snake_case)]
pub fn Case(cond: Expr, result: Expr) -> Expr {
  Expr(result.ty().clone(),
       ExprKind::Case(Box::new(cond), Box::new(result)))
}
...
pub mod visitor {
  ...
  pub fn accept_by_default<T>(v: &mut T, e: &Expr)
    where T: Visitor + Sized
  {
    match *e.kind() {
      ...
      ExprKind::Cmp(_, ref l, ref r) => {
        v.accept(l);
        v.accept(r)
      }
      ExprKind::Arithm(_, ref l, ref r) => {
        v.accept(l);
        v.accept(r)
      }

      ExprKind::Fn(_, ref args) => {
        for e in args.iter() {
          v.accept(e)
        }
      }

...
pub struct ConstVector<'a> {
  value: [u8; 16],
  data_ty: DataTy,
  _marker: marker::PhantomData<&'a ()>
}

impl<'a> ConstVector<'a> {
  pub fn new(datum: &'a Datum) -> ConstVector {
    ...
    ConstVector {
      ...
      _marker: marker::PhantomData // to keep the lifetime of BLOB values
    }
  }
}

impl<'a> HasDataTy for ConstVector<'a> {
  fn data_ty(&self) -> &DataTy {
    &self.data_ty
  }
}

impl<'a> Vector for ConstVector<'a> {
  #[inline]
...
fn test_const_vector() {
  let bool_value = Datum::Bool(true);
  let bool_vec: &Vector = &ConstVector::new(&bool_value);
  assert_eq!(Ty::Bool, bool_value.ty());
  assert_eq!(Ty::Bool, bool_vec.data_ty().ty());
  assert_eq!(1, bool_vec.size());
  assert_eq!(true, *first_value(bool_vec));

  let int1_value = Datum::Int1(7);
  let int1_vec: &Vector = &ConstVector::new(&int1_value);
  assert_eq!(Ty::Int1, int1_value.ty());
  assert_eq!(Ty::Int1, int1_vec.data_ty().ty());
  assert_eq!(1, int1_vec.size());
  assert_eq!(7, *first_value(int1_vec));

  let int2_value = Datum::Int2(17);
  let int2_vec: &Vector = &ConstVector::new(&int2_value);
  assert_eq!(Ty::Int2, int2_value.ty());
  assert_eq!(Ty::Int2, int2_vec.data_ty().ty());
  assert_eq!(1, int2_vec.size());
  assert_eq!(17, *first_value(int2_vec));

  let int4_value = Datum::Int4(178910);
  let int4_vec: &Vector = &ConstVector::new(&int4_value);
  assert_eq!(Ty::Int4, int4_value.ty());
  assert_eq!(Ty::Int4, int4_vec.data_ty().ty());
  assert_eq!(1, int4_vec.size());
  assert_eq!(178910, *first_value(int4_vec));

  let int8_value = Datum::Int8(981627341);
  let int8_vec: &Vector = &ConstVector::new(&int8_value);
  assert_eq!(Ty::Int8, int8_value.ty());
  assert_eq!(Ty::Int8, int8_vec.data_ty().ty());
  assert_eq!(1, int8_vec.size());
  assert_eq!(981627341, *first_value(int8_vec));

  let float4_value = Datum::Float4(3.14f32);
  let float4_vec: &Vector = &ConstVector::new(&float4_value);
  assert_eq!(Ty::Float4, float4_value.ty());
  assert_eq!(Ty::Float4, float4_vec.data_ty().ty());
  assert_eq!(1, float4_vec.size());
  assert_eq!(3.14f32, *first_value(float4_vec));

  let float8_value = Datum::Float8(87123.1452f64);
  let float8_vec: &Vector = &ConstVector::new(&float8_value);
  assert_eq!(Ty::Float8, float8_value.ty());
  assert_eq!(Ty::Float8, float8_vec.data_ty().ty());
  assert_eq!(1, float8_vec.size());
  assert_eq!(87123.1452f64, *first_value(float8_vec));

  let text_value = Datum::Text("hyunsik".to_string());
  let text_vec: &Vector = &ConstVector::new(&text_value);
  assert_eq!(Ty::Text, text_value.ty());
